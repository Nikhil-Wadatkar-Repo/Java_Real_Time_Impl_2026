...
import com.enterprise.payroll.department.dto.ErrorResponse;
...
/**
 * Centralized exception -> HTTP response translation. Never leaks a stack
 * trace to the client; full details go to the log with the correlation id
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler({OptimisticLockConflictException.class, OptimisticLockException.class})
    public ResponseEntity<ErrorResponse> handleOptimisticLock(RuntimeException ex, HttpServletRequest request) {
        return build(HttpStatus.CONFLICT, "OPTIMISTIC_LOCK_CONFLICT",
                "The record was modified by another request. Please refresh and try again.", request, null);
    }
